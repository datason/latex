...
1) $\hat{\beta}_j|X \sim N(\beta_j, {Var}(\hat{\beta}_j|X))$\\

2) $\frac{\hat{\beta}_j-\beta_j}{{Var}(\beta_j|X)}=\frac{\hat{\beta}_j-\beta_j}{\kappa(\beta_j)}$\\

3)Если $\exists$ 2 вложенные модели: "короткая" - частный случай "длинной", то:\\
$$\frac{\frac{RSS_{кор}-RSS{длин}}{k_{длин}-k_{кор}}}   {\frac{RSS_{длин}}{n-k_{длин}}} \sim F_{k_{длин}-k_{кор},n-k_{длин}}$$\\

Задача
Дано:
Модель А: $\hat{y}_i=2.7 - 3.2x_i, R^2=0.6\\
                    (1.1) (0.6)  \\$
Модель В: $\hat{y}_i=3.2 - 2.8x_i + 4.2z_i + 0.1x_i^2, R^2=0.7\\
                    (1.2) (0.7)    (1.1)    (0.5)$\\
В скобках ниже указаны стандартные ошибки\\

Задание:
Проверить гипотезу на стат. значимость при уровне доверия $\alpha = 5\% $  \\
$H_0$: верна А.\\
